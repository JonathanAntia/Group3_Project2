...
function handleResultButtonSubmit(){
    ...
    printUserSelection(weightCriteriaProvided);
}

function printUserSelection(weightCriteriaProvided){
    console.log(weightCriteriaProvided)
}
...
d3.json(`static/data/data.json`).then((data)=>{
    ...
    Morris.Bar({
        ...
        data: tableData,
        xkey: 'neighborhood',
        ykeys: ['sales', 'crime', 'school', 'acreage', 'sqft', 'flood', 'valuation'],
        labels: ['Sales Index', 'Crime Index', 'School Rating Index', 'Acreage Index', 'Sqft Index', 'Flood Index', 'Valuation Index'],
        barColors: ['blue','orange','green','red','purple','brown','pink']
      });
});  
//////////////////////////////////////////////////////////
// UPDATE PLOTLY INTERACTIVE BAR CHART PER USER SELECTION
//////////////////////////////////////////////////////////
...
function chooseParameter (data, parameter){
    switch(parameter){
        case "SalesIndex":
            return data.map(item => item.SalesIndex);
        case "CrimeIndex":
            return data.map(item => item.CrimeIndex);
        case "SchoolRating":
